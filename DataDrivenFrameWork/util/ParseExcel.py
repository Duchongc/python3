...
class PatseExcel(object):
    ...
    def getColumn(self,sheet,colNo):
        # 获取sheet中某一列,返回的事这一列所有数据内容组成的tuple
    ...
    def getCellOfObject(self, sheet, coorfdinate=None, row=None, colsNo=None):
        # 获取某个单元格的对象,可以根据单元格所在位置的数字索引
        # 也可以直接根据Excel中单元格的编码及坐标
        # 如
        if coorfdinate != None:
            try:
                return sheet.cell(coorfdinate=coorfdinate)
            except Exception, e:
                raise e
        elif coorfdinate == None and rowNo is not None and colsNo is not None:
            try:
        ...
        if coordinate is not None:
            try:
                ...
                if style is not None:
                    sheet.cell(corrdinate=coordinate).font = Font(color=self.RGBDict[style])
...
if __name__ == '__main__':
    pe = PatseExcel()
    pe.loadWorkBook(u'D:\\toors\\python\\DataDrivenFrameWork\\testData\\126邮箱.xlsx')
    ...
    pe.writeCell(sheet,u'我爱月月',rowNo = 10,colsNo = 10)
